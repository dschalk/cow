...
var Component2 = React.createClass({
  ...
  render: function() {
    ...
    return a >>= k                              = k a
    m >>= return                                = m
    m >>= (<lambda backslash>x -> k x >>= h)    = (m >>= k) >>= h
        `}
        </Markdown>
      </div>
    );
  ...
  render: function() {
    return (
      <div style={{fontSize: 20}} >
        <Markdown>
        {`
          this.obDouble({a:10,b:20,c:30}); this.m.bnd(this.fmSubtract1)}}
        `}
        </Markdown>
      </div>
    );
...
@reactiveComponent class B2 extends React.Component {
  constructor(props) {
    ...
    this.m3 = fibData.m3;
    this.ax = () => {this.m.x.observe(function(a,b) {return a})};
    this.bx = () => {this.m.x.observe(function(a,b) {return b})};
...
onTextareaChange2 = (event) => {
  this.setState({
    content2: MTRC(event.target.value).tree
  });
}

onTextareaChange3 = (event) => {
  this.setState({
    content3: MTRC(event.target.value).tree
  });
}
  ...
  comment1 = () => {
    return " mAdd = (z,w,mon) => { return mon.ret(z + w); } "
  }

  comment3 = () => {return "this.ma1.bnd(this.fibCalc).bnd(this.m2.ret)"}
...
Example 3, illustrating the left identity property of monads. This result is identical to the result in 'Example 2'.
  <br />
          The result of running "obDouble" on an object containing 10, 20, and 30 is displayed in mo1 (on the right) after clicking the button below.

          <br /> <br />

          <button style={this.style8(cr27,cr270,cr28)} onClick={() => {this.mo1 =  this.obDouble({a:10,b:20,c:30}); this.m.bnd(this.fmSubtract1)}}
            onMouseEnter={() => {this.mouse[87] = 'blue', this.mouse[870] = 'lightblue', this.mouse[88] = 'yellow'}}
            onMouseLeave={() => {this.mouse[87] = '#000', this.mouse[870] = 'darkred', this.mouse[88] = 'burlywood'}}
            > this.obDouble(object containing values of 10, 20, and 30); this.m.bnd(this.fmSubtract1)}}
            <br /><br />
          </button>
          <br /> <br />
...
Example 8, set mo1 and mo2.
          ...
          <br /><br />
Example 9, test.
          <button style={this.style8(cr137,cr1370,cr138)} onClick={() => {this.mo1.ret({a: 3, b: 3, c: 3}).bnd(this.mo2.ret).bnd(this.obCube).bnd(this.mo3.ret).bnd(this.obReplace, ({a: 'Done', b: 'b', c: 'c'}))}}
            onMouseEnter={() => {this.mouse[137] = 'blue', this.mouse[1370] = 'lightblue', this.mouse[138] = 'yellow'}}
            onMouseLeave={() => {this.mouse[137] = '#000', this.mouse[1370] = 'darkred', this.mouse[138] = 'burlywood'}}
            >
...
<Component6 />
          ...
          <br /><br />
          <h3> Associativity </h3>
          <button style={this.style8(cr137,cr1370,cr138)} onClick={() => {this.m.bnd(this.mAdd,1,this.m).bnd(this.fmSquare)}}
            onMouseEnter={() => {this.mouse[137] = 'blue', this.mouse[1370] = 'lightblue', this.mouse[138] = 'yellow'}}
            onMouseLeave={() => {this.mouse[137] = '#000', this.mouse[1370] = 'darkred', this.mouse[138] = 'burlywood'}}
         ...
         <br /><br />
          ...
          <br /><br />
          <button style={this.style8(cr87,cr870,cr88)} onClick={() => {this.m2.bnd(this.reset_3)}}
            onMouseEnter={() => {this.mouse[87] = 'blue', this.mouse[870] = 'lightblue', this.mouse[88] = 'yellow'}}
            onMouseLeave={() => {this.mouse[87] = '#000', this.mouse[870] = 'darkred', this.mouse[88] = 'burlywood'}}
            > Click to reset m3.
          </button>
          ...
          <button style={this.style8(cr77,cr770,cr78)} onClick={() => {this.m3.bnd(this.fibo)}}
            onMouseEnter={() => {this.mouse[77] = 'blue', this.mouse[770] = 'lightblue', this.mouse[78] = 'yellow'}}
            onMouseLeave={() => {this.mouse[77] = '#000', this.mouse[770] = 'darkred', this.mouse[78] = 'burlywood'}}
            > Click to observe 'this.m3.bnd(this.fibo). Explained below:
          </button>
          <br /><br />

    I don't know if manipulating mobservable reactive entities in this manner will have any practical value. Maybe chaining, as in
    "this.m.bnd(this.mAdd,1,this.m).bnd(this.mAdd,3,this.m).bnd(this.fmSquare) .bnd(this.m2).bnd(this.mAdd,1,this.m).bnd(this.fmSquare).bnd(this.mAdd,7,m)"(see above), will be useful, perhaps as an alternative to promises for assuring that sychronous operations execute in a specified order.
    <br /><br />
...
<Component2 />
        ...
        <h2 style={{textAlign: 'center' }} >Working With Text</h2>
          <button style={this.style8(cr77,cr770,cr78)} onClick={() => {this.m.ret('Hello')}}
            onMouseEnter={() => {this.mouse[77] = 'blue', this.mouse[770] = 'lightblue', this.mouse[78] = 'yellow'}}
            onMouseLeave={() => {this.mouse[77] = '#000', this.mouse[770] = 'darkred', this.mouse[78] = 'burlywood'}}
            >
            this.m.ret('Hello')
      ...
      <br /><br />
       The next button puts "World" in monad m2.
          <br /><br />
          <button style={this.style8(cr97,cr970,cr98)} onClick={() => {this.m2.ret(' World')}}
            onMouseEnter={() => {this.mouse[97] = 'blue', this.mouse[970] = 'lightblue', this.mouse[98] = 'yellow'}}
            onMouseLeave={() => {this.mouse[97] = '#000', this.mouse[970] = 'darkred', this.mouse[98] = 'burlywood'}}
            >
            this.m2.ret(' World')
          ...
          <br /><br />
          Clicking the next button runs "this.m.bnd(this.mAdd, this.m2(),this.m3) on m, then sends the result to m3 with ".bnd(this.m3)", and finally resets m with ".bnd(this.reset_1)".
          <br /><br />
          <button style={this.style8(cr87,cr870,cr88)} onClick={() => {this.m.bnd(this.mAdd, this.m2.x(),this.m3).bnd(this.reset_1)}}
            onMouseEnter={() => {this.mouse[87] = 'blue', this.mouse[870] = 'lightblue', this.mouse[88] = 'yellow'}}
            onMouseLeave={() => {this.mouse[87] = '#000', this.mouse[870] = 'darkred', this.mouse[88] = 'burlywood'}}
            >
            this.m.bnd(this.mAdd, this.m2.x(),this.m3)
      </button>
